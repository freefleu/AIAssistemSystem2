...
def index(request):
    ctx={}
    ctx['rlt'] = request.session['stu_name'] + '登陆成功!'
...
def getcourse(request):
    ...
    if request.POST.get('course_id123'):#选课操作
        #return HttpResponse("<p>收到POST</p>")
        #return HttpResponse(request.GET['course_id'][1:11])
        selectcourse_id = request.POST.get('course_id123')
    ...
    if request.POST.get('course_id789'):#组队界面
